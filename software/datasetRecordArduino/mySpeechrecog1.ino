...
bool PollBands(bool init)
{
  ...
  for (curSegment = 0; curSegment < NumSegments; curSegment++) {
    ...
    while (millis() - prevTime < SegmentSize)
    {
      ...
      val1 = ADCL; // ADCL must be read first
      val2 = ADCH;
...
{
  byte seg, i, band, low, high;

  Serial.write("a");
  for (seg = 0; seg < NumSegments; seg++) {
    for (band = 0; band < nBand + 1; band++) {
      high = highByte(CurBandData[seg][band]);
      low = lowByte(CurBandData[seg][band]);
      Serial.write(high);
      Serial.write(low);
      Serial.write(" ");
    }
